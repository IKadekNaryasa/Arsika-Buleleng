...
<div class="row">
    <div class="card mb-4">
        <h5 class="card-header text-center">Edit Kode Klasifikasi {{ $klasifikasi->kode }}</h5>
        <div class="card-body">
            <form action="{{ route('admin.klasifikasi.update',$klasifikasi->id) }}" method="post" id="klasifikasiForm">
                @csrf
                @method('PUT')
...
<script>
    const klasifikasiForm = document.getElementById('klasifikasiForm');

    klasifikasiForm.addEventListener('submit', function(e) {
        loadingOverlay.classList.remove('d-none');
        klasifikasiForm.classList.add('form-disabled');
        submitBtn.disabled = true;
        submitText.textContent = 'Menyimpan klasifikasi, mohon tunggu...';
